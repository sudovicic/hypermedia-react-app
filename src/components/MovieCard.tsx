    ...
    <div className="card lg:card-side lg:max-w-xl bg-base-100 shadow-xl">
      <figure className="card-body">
        <img
          ...
          alt={movie['#IMG_POSTER'] ? t('alt_thumbnail', { resource: movie['#TITLE'] }) : t('no_preview_image')}
        />
      </figure>
      <div className="card-body justify-between gap-3">
        ...
        <div>
          ...
          <p className="flex-grow-0 line-clamp-6">
            {movie['#IMDb_SHORT_DESC'] ? movie['#IMDb_SHORT_DESC'] : t('no_description')}
          </p>
        </div>
        <div className="card-actions justify-between">
          {movie['#YEAR'] && <span>{new Date(Date.parse(movie['#YEAR'].toString())).getFullYear()}</span>}
          {movie['#MARINTG'] && <div className="badge badge-outline">{movie['#MARINTG']}</div>}
        </div>
      </div>
    </div>
  );
